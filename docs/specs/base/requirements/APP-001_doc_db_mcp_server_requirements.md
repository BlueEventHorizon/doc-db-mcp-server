...
| ID     | 要件                                                                                                           |
| ------ | -------------------------------------------------------------------------------------------------------------- |
| EXP-01 | TTL（最終アクセスからの経過時間）: 指定期間アクセスのない KEY を自動削除する。当該 KEY に対して FNC-006 SYN-08 の KEY 単位排他ロックを取得してから削除する（`sync_documents` 処理中の KEY を削除しないため） |
| EXP-02 | LRU（容量制限）: 総チャンク数またはストレージ使用量が上限を超えた場合、最終アクセスが最も古い KEY から削除する。削除対象 KEY ごとに EXP-01 と同様の排他ロックを取得する                              |
...
`upsert_documents` は「渡されたドキュメントを追加・更新する」追加専用の操作であり、クライアント側でファイルが削除された場合にそれを検出してインデックスから除去する仕組みを持たない。その結果、削除済みファイルの record が無期限にインデックスへ残留し、検索結果に汚染が生じる問題がある（また EXP-01/02 の TTL/LRU は `last_accessed_at` という推測的な指標に基づく KEY 単位の自動廃棄であり、削除済み事実を確定的に検出する手段ではない。TBD-009 で提起されていた「未使用 series の自動廃棄」はこの FNC-006 の仕組みで解決する）。
...
| SYN-03 | 処理完了後、当該 key・series に既存登録されているが `documents` に含まれない path を検出し、即座に削除せず削除予約として記録する                                          |
| SYN-04 | 過去に削除予約された path が、その後の `sync_documents` 呼び出しの `documents` に再度含まれた場合、削除予約を解除する（誤操作からの自己修復）。当該 key・series に GC-01 由来の **series 全体の削除予約**が存在する場合も、`sync_documents` が呼ばれた時点でその series はまだ使われていると判断し、series 全体の削除予約を解除する |
...
| GC-02 | series 単位（GC-01 由来）・path 単位（SYN-03 由来）いずれの削除予約も、サーバー起動時に一括して物理削除する。物理削除は既存の DEL-01（path 単位）・DEL-03（series 単位）と同じ安全条件（他 series が参照する record は保持）に従う |
...
| TBD-009 | series の廃棄ポリシー: EXP-01/02 の廃棄単位を KEY のみとするか series まで拡張するか。未使用 series（feature ブランチ削除後など）の自動廃棄が必要かどうかの意思決定が必要 | FNC-006（GC-01〜04）で解決: TTL/LRU の推測ベース拡張ではなく、クライアント確定情報に基づく削除予約＋起動時スイープ方式を採用（2026-07-03） |
...
| 2026-07-03 | k2moons | FNC-006 追加: desired-state 同期ジョブ（`sync_documents`/`get_sync_status`、SYN-01〜07）と削除予約の起動時ガベージコレクション（`schedule_delete_series`、GC-01〜04）。TBD-009 を解決。既存の TTL/LRU（EXP-01/02）は推測ベースで仕事用途の削除には不向きと判断し変更せず、確定情報に基づく別経路として新設 |
| 2026-07-03 | k2moons | FNC-006 レビュー対応: SYN-08 追加（同一 key・series の書き込み系ツールを sync_documents と排他実行）。SYN-04 に series 全体削除予約の自己修復を追記。SYN-06 に job_id エラー応答を明記。出力仕様（sync_documents/get_sync_status/schedule_delete_series）を新設。GC-05 追加（ジョブ用 context をリクエスト context から分離、シャットダウン時は failed 化） |
| 2026-07-04 | k2moons | FNC-006 レビュー対応(追補): SYN-08 の排他対象漏れを修正。`delete_index`（MNG-02）・EXP-01（TTL）・EXP-02（LRU）による KEY 全体削除が排他対象から漏れていたため対象に追加し、ロック粒度を KEY 単位に明確化（詳細は DES-003）。EXP-01/EXP-02 の要件文に排他ロック取得を明記。`schedule_delete_series` の出力を `{key, series, already_scheduled}` に具体化 |
