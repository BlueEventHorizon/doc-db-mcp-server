...
| series     | string  | -    | 絞り込む時系列キー。省略時は全 series を横断検索する。**series は opaque な絞り込み軸であり、サーバーは登録状態の検証を行わない。KEY に登録されていない series を指定してもエラーにはならず、該当 0 件の成功応答を返す（安定契約。将来もエラー化しない）**。「未同期」と「同期済みだが 0 件」の切り分けはクライアント側の責務（MNG-01 参照。Issue #8） |
...
| 2026-08-01 | k2moons | FNC-003 の series パラメータに未登録 series の挙動を安定契約として明文化。series は opaque な絞り込み軸であり登録状態の検証を行わないため、未登録 series の指定はエラーではなく該当 0 件の成功応答となる（将来もエラー化しない）。「未同期」と「同期済みだが 0 件」の切り分けは MNG-01 記載のとおりクライアント側の責務。Issue #8                                                                                                                                                                               |
