...
- 既存の `list_indexes` / `schedule_delete_series` / `sync_documents` の機能はそのまま存在する
- 既存の `delete_index`（即時物理削除）は本要件のゴミ箱投入操作に置き換えられ廃止される。KEY の削除経路をゴミ箱投入（本要件）に一本化することで、削除が常に猶予期間を経由することを保証する
...
- 既存の `delete_index`（即時物理削除）ツールは廃止する。KEY の削除は必ず FNC-009 のゴミ箱投入を経由し、猶予期間なしに即座に物理削除される経路を残さない
...
- `list_indexes` の一覧は、ゴミ箱状態の KEY を含まない（利用可能な KEY のみを返す）。ゴミ箱状態の KEY は FNC-010 の一覧でのみ確認できる
...
- 保持期間は `doc-db.yaml` の `trash.retention_days` で変更できる。最終処分のチェック間隔は `trash.interval_seconds` で変更できる（既存の `expiry:` セクションは廃止し `trash:` セクションに置き換える）
