...
1. **別内容での復活時**: `upsertOne` の DIF-03 経路は `UpsertRecord` 直後に必ず `CleanOtherSeries`（`internal/mcp/mcp.go:401` → `cleanOtherSeriesLocked`）を呼ぶ。同メソッドは **series 条件なしで同一 key+path の全 record**（新 record を除く）を走査し、`series_keys` が空の record を物理削除する。つまり旧 orphan は新 record 作成と同一の `upsertOne` 呼び出し内で既存機構が物理削除するため、その後 `ClearPendingDeletion` で予約が消えても回収漏れにならない（同一内容での復活 = DIF-02 経路は record 再利用なので orphan 自体が生じない）
...
**`ClearPendingDeletion` の実行条件 [MANDATORY]**: `sync_documents` の `fn` 内では、`documents` に含まれる path のうち **`upsertOne` が成功（processed または skipped）した path のみ** `ClearPendingDeletion` を呼ぶ。失敗（failed）した path の予約は保持する。失敗 path は新 record が作られておらず `CleanOtherSeries` にも到達していないため、予約まで解除すると旧 orphan の回収手段が失われる（残穴の防止）。なお `upsertOne` 成功後の `CleanOtherSeries` 個別失敗（警告扱いで継続する既存挙動）により orphan が残るケースは、次回同 path の upsert 時に再回収される既存の自己修復挙動に委ねる（本 feature 固有のリスクではない）。
...
- 呼び出し元一覧と囲み方（`fn` は「対象 KEY に対する Store 呼び出し一式」を指し、実際には単一メソッド呼び出しとは限らない。例えば `upsert_documents` は複数ドキュメント・複数チャンクに対し `UpsertRecord` を繰り返し呼ぶが、そのハンドラ処理全体を 1 回の `WithKeyLock` で囲む）:
  ...
  - `sync_documents`（新規ゴルーチン）: **1 回の `WithKeyLock` で desired-state 判定全体（documents 処理 → path 一覧取得 → series 切り離し → 削除予約の記録・解除）を囲む**。`fn` の内部では `upsertOne` / `DetachSeriesFromPath` / `MarkDocumentForDeletion` / `ClearPendingDeletion` を直接呼び、これらの中から `WithKeyLock` を再度呼ばない（これらのメソッド自体が `WithKeyLock` を持たないため、構造的に再入は起こり得ない）
...
sequenceDiagram
    ...
    S->>D: "documentsに含まれupsertOne成功したpathのみClearPendingDeletion（自己修復。失敗pathの予約は保持）"
