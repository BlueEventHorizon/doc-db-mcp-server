...
`DeleteSeriesAll`（`internal/store/store.go:742`）は series 単位スイープで無改造再利用する（「`series_keys` 除去後 `COUNT(*)=0` の場合のみ物理削除、他 series が残る record は保持」という既存の安全な不変条件を持つ）。`DeleteSeries`（同 664）は本 feature のスイープでは**使用しない**（path 単位は `DeleteOrphanRecords`。上記 Sweep 行の理由参照）。既存 `delete_documents` ハンドラでの利用は従来通り（§3.5.2 の `WithKeyLock` ラップ対象）。
