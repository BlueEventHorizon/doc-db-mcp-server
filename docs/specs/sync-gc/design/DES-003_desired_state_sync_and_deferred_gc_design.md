...
3. **削除予約の起動時スイープ**: series 単位（`schedule_delete_series` 経由）・path 単位（`sync_documents` で切り離し済みの orphan record）いずれの削除予約も、サーバー起動時に一括して物理削除する。series 単位は既存の `DeleteSeriesAll`（他 series が参照する record は保持する安全な不変条件込み）を無改造で再利用し、path 単位は `DeleteOrphanRecords`（orphan-only。stale な予約行が残っていても live record を壊さない、§3.3）で処理する
...
| モジュール名                                                    | 責務                                                                                                                                                                                 | 依存                               |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------- |
| `internal/store`（`pending_deletions` テーブル + 新規メソッド） | series 紐付けの即時切り離し（`DetachSeriesFromPath`）・orphan 回収（`DeleteOrphanRecords`）・削除予約の記録・取得・解除・起動時スイープの実行                                        | 既存 `DeleteSeriesAll`             |
...
| `DeleteOrphanRecords(ctx, key, path string) (removed int, err error)`                        | 指定 key+path の record のうち **`series_keys` が 0 件のもののみ**を物理削除する（chunks / embeddings / BM25 整合含む。既存 `deleteRecordWithBM25Tx` を再利用）。series の紐付きが残る record には一切触れないため冪等かつ常に安全。呼び出し元は 2 箇所: (a) `sync_documents` の `fn` 内で `ClearPendingDeletion` の**直前**（`CleanOtherSeries` の個別失敗（警告扱いで継続する既存挙動、mcp.go:401-404）が起きていても orphan を決定的に回収する。下記 [MANDATORY] 参照）、(b) `SweepPendingDeletions` の path 単位処理（起動時）。record 削除を伴うため doc_count を更新する。単一トランザクション + `s.mu` 直列化。`WithKeyLock` は内部で取得しない（(a) は呼び出し元の `fn` が保持済み・(b) は起動時専用のため不要、§3.5.2）                                                                                                                                                                                                                                                                                                                                                                                   |
| `ClearPendingDeletion(ctx, key, series, path string) error`                                  | 該当行を削除（SYN-04 の自己修復に使用）。`path=""` を渡すと series 全体の削除予約（GC-01 由来）を解除する                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `ListPendingDeletions(ctx, key, series string) (paths []string, seriesWide bool, err error)` | 当該 key+series の削除予約を 1 回で取得する（`paths` = path 単位予約の一覧、`seriesWide` = series 全体予約（GC-01 由来、`path=''` 行）の有無）。`sync_documents` の `fn` 冒頭で呼び、補償+解除（下記 [MANDATORY]）の対象 path と SYN-04 の series 全体予約解除の要否を判定する。読み取りのみ                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `SweepPendingDeletions(ctx context.Context) (processed int, errs []error)`                   | `pending_deletions` 全行を取得し、`path=''` なら既存 `DeleteSeriesAll(ctx, key, series)`、**それ以外（path 単位）なら `DeleteOrphanRecords(ctx, key, path)` を呼ぶ**。path 単位の行は「この path の orphan（どの series からも参照されない record）を回収せよ」という物理削除予約であり、`DeleteOrphanRecords` は series 紐付きが残る record に一切触れないため、**stale な予約行が残っていても live record を壊さない**（予約行の作成後に record が復活・再紐付けされていた場合は 0 件処理の冪等動作で行だけが消える。`DeleteSeries` を使うと復活済み record から series を剥がして削除し得るため使わない）。成功した行は削除。個別失敗はログ＋`errs`に集約して継続（silent failure 禁止方針）。行の消し忘れは次回起動時に再試行されるだけで安全（両関数とも対象が既に無ければ 0 件処理で冪等）。**起動時（サーバーがまだ MCP リクエストを受け付ける前）にのみ呼ばれる前提**であり、`WithKeyLock` を取得しない（並行する書き込みが存在しない時間帯のため不要）。将来、起動時以外（手動トリガー等）でスイープを実行する変更を加える場合は、この前提が崩れるため各行ごとに `WithKeyLock` で囲むよう設計を見直すこと |

`DeleteSeriesAll`（`internal/store/store.go:742`）は series 単位スイープで再利用する（「`series_keys` 除去後 `COUNT(*)=0` の場合のみ物理削除、他 series が残る record は保持」という既存の安全な不変条件を持つ）。`DeleteSeries`（同 664）は本 feature のスイープでは**使用しない**（path 単位は `DeleteOrphanRecords`。上記 Sweep 行の理由参照）。既存 `delete_documents` ハンドラでの利用は従来通り（§3.5.2 の `WithKeyLock` ラップ対象）。

**stale 予約の発生源遮断 [MANDATORY]**: `DeleteKey` は当該 KEY の全 `pending_deletions` 行を、`DeleteSeriesAll` は当該 key+series の **series-wide 予約（`path=''` センチネル）のみ**を、**それぞれ本体の削除と同一トランザクションで除去する**。これを行わないと、`schedule_delete_series(K, s)` → `delete_index(K)`（または `delete_series(K, s)`）→ 同名 KEY/series の再作成 → 再起動、の順序で stale な series-wide 予約が残存し、起動時スイープの `DeleteSeriesAll` が**再作成された新データを削除する**（series-wide 行は live record を対象にするため発生源で断つ必要がある）。なお `sync_documents` 経由の series-wide 予約解除（SYN-04）はこの遮断とは独立に機能する。

除去範囲は両メソッドで**非対称**であり、これは意図的な設計である:

- `DeleteKey` は `records WHERE key=?` で **orphan record（`series_keys` 0 件）を含む全 record を物理削除する**ため、当該 KEY の予約はすべて（path 単位含め）無意味になる → 全行除去してよい
...
- 削除予約中の path を `sync_documents` を経由せず既存 `upsert_documents` で復活させた場合、SYN-04 の予約解除は行われず予約行が残る。ただし起動時スイープの path 単位処理は `DeleteOrphanRecords`（orphan-only、series 紐付き record には不触）であるため、**復活済みの record が壊れることはなく**、stale な予約行は起動時に 0 件処理で除去されるだけで無害である。予約解除を即時に行いたい場合は復活も `sync_documents` で行うこと
- **cross-series の自己修復猶予は保証されない**: SYN-04 の「API 課金ゼロ自己修復」は、当該 orphan が回収される前に同一 series の再 sync が行われた場合の保証である。`DeleteOrphanRecords` は series を見ずに同一 key+path の orphan を全回収し、既存 `CleanOtherSeries` / `AppendAndCleanSeries` も同一 key+path の空 record を掃除するため、**別 series が同じ key+path を upsert / sync すると、他 series の自己修復用に残していた orphan も回収される**。その後に元の series が同一内容で再 sync した場合は通常の新規登録として Embedding を再計算する（動作は常に正しく、失われるのは課金ゼロの猶予のみ）。これは本 feature 固有ではなく、既存掃除機構と整合した挙動である
...
2. **同一 key+path に複数 orphan（複数 content_hash）が並ぶ場合**: スイープが呼ぶ `DeleteOrphanRecords(key, path)` は **path 配下の全 record** を走査し、`series_keys` が 0 件のものを全て物理削除する。pending 行は「この path の orphan を回収せよ」という指示であり、個々の record を識別する必要がない（どの 1 行のスイープでも当該 path の全 orphan が回収される）
...
**`ClearPendingDeletion` の実行条件 [MANDATORY]**: `sync_documents` の `fn` 内では、冒頭で `ListPendingDeletions(ctx, key, series)` により当該 key+series の削除予約を 1 回で取得しておき、`documents` に含まれる path のうち **`upsertOne` が成功（processed または skipped）し、かつ削除予約が存在する path** について、以下の 2 段階で予約を解除する:

1. **`DeleteOrphanRecords(ctx, key, path)` を先に実行**する。`upsertOne` 内の `CleanOtherSeries` は個別失敗しても警告扱いで `Processed++` される既存挙動（mcp.go:401-404）のため、「processed = 旧 orphan は掃除済み」とは限らない。この補償呼び出しにより、`CleanOtherSeries` の成否に関係なく旧 orphan が決定的に回収される（成功していた場合は 0 件処理の冪等動作）
...
失敗（failed）した path の予約は保持し、上記 2 段階も実行しない。失敗 path は新 record が作られておらず、予約まで解除すると旧 orphan の回収手段が失われる（残穴の防止）。

補足（安全性の根拠）: 仮に実装が予約有無の判定を省き、成功 path 全件に `DeleteOrphanRecords` を呼んでも**事故にはならない**。`upsertOne` の成功は DIF-02（`AppendAndCleanSeries`）/ DIF-03（`CleanOtherSeries`）いずれの経路でも同一 key+path の空 record を既に物理削除しており、この呼び出しはその冪等な再実行にすぎない（既存機構が消さない orphan を消すことはない）。`ListPendingDeletions` で対象を絞るのは正しさのためではなく、無駄な呼び出しの削減と実装意図の明確化のためである。
...
- 呼び出し元一覧と囲み方（`fn` は「対象 KEY に対する Store 呼び出し一式」を指し、実際には単一メソッド呼び出しとは限らない。例えば `upsert_documents` は複数ドキュメント・複数チャンクに対し `UpsertRecord` を繰り返し呼ぶが、そのハンドラ処理全体を 1 回の `WithKeyLock` で囲む）:
  ...
  - `internal/mcp` の `delete_documents` ハンドラ: `store.WithKeyLock(ctx, key, func() error { /* HasRecord による存在チェック（warning 構築）から DeleteSeries まで */ })`。**存在チェックをロック外に置いてはならない**: `sync_documents` がロック保持中に作成する path を「存在しない」と誤判定してブロックせず即完了し、削除要求を取りこぼす（TOCTOU。SYN-08 は判定を含む処理全体の排他を要求する）
  ...
  - `sync_documents`（新規ゴルーチン）: **1 回の `WithKeyLock` で desired-state 判定全体（documents 処理 → path 一覧取得 → series 切り離し → 削除予約の記録・解除）を囲む**。`fn` の内部では `upsertOne` / `ListPendingDeletions` / `DetachSeriesFromPath` / `MarkDocumentForDeletion` / `DeleteOrphanRecords` / `ClearPendingDeletion` を直接呼び、これらの中から `WithKeyLock` を再度呼ばない（これらのメソッド自体が `WithKeyLock` を持たないため、構造的に再入は起こり得ない）
...
**前提条件**: `key`・`series` は既存 KEY に対する呼び出し。`documents` は当該 key・series の完全な現在状態であること（クライアントの責務）。**空リストも正当な desired-state として受理する**（「この series に現存ファイルがない」の宣言。既存 path は全て切り離し + orphan 予約となり、誤送信でも同一内容の再 sync で Embedding 再計算なしに復元できる。即時物理削除する `delete_series` への誘導は SYN-03/04 の自己修復性を失うため行わない）。削除予約中の path の復活は `sync_documents` で行うこと（§3.3 既知の制約）。
...
| コンポーネント          | ファイルパス                      | 用途                                                                                                                                                                                |
| ----------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DeleteSeries`          | `internal/store/store.go:664`     | 既存 `delete_documents` ハンドラの `WithKeyLock` ラップ対象（無改造）。**スイープでは使用しない**（stale 予約行が live record を壊すため。path 単位は `DeleteOrphanRecords`、§3.3） |
...
- **単体テスト対象**（`internal/store/store_test.go`）:
  ...
  - **stale 予約行の無害性検証（スイープの回帰テスト）**: path 単位の削除予約が残ったまま当該 record が復活（series 再紐付け）した状態でスイープを実行しても、**live record（chunks / embeddings 含む）が保持され**、予約行のみが除去されること。`DeleteSeries` ベースのスイープでは復活済み record が破壊されるため、この退行を直接検出する
  - `ListPendingDeletions` が path 単位予約の一覧と series 全体予約の有無を正しく返すこと（予約なし・path のみ・series 全体のみ・両方の 4 状態）
...
| 2026-07-04 | 1.9        | レビュー指摘対応（stale 予約行による live record 破壊リスク）: 起動時スイープの path 単位処理を `DeleteSeries` から `DeleteOrphanRecords` に変更。path 単位予約は作成時点で対象が orphan（`orphaned=true` の場合のみ Mark）であり series 剥がしは本来不要で、`DeleteSeries` の series 剥がしは stale 予約行（Clear 失敗・upsert_documents 経由復活・補償失敗後の再試行）に対して復活済み record を破壊する危険部分だけが残っていた。orphan-only 化により stale 行は 0 件処理で無害となり、既知の制約だった「upsert_documents 経由復活の破壊リスク」も解消（§3.3 更新）。予約有無の判定 API 不在の指摘に対し `ListPendingDeletions(ctx, key, series)` を新設し fn 冒頭で 1 回取得する設計に明確化。「成功 path 全件への無条件 `DeleteOrphanRecords` でも事故にならない」根拠（DIF-02/03 の既存掃除の冪等な再実行）を安全性の補足として明記。§6 に stale 予約行の無害性検証・`ListPendingDeletions` 単体テストを追加 |
| 2026-07-05 | 1.10       | レビュー指摘対応（保証範囲の表現調整）: §3.3 既知の制約に「cross-series の自己修復猶予は保証されない」を追加（`DeleteOrphanRecords` / 既存 `CleanOtherSeries` は series を見ずに同一 key+path の orphan を回収するため、別 series の upsert / sync が先行すると他 series の自己修復用 orphan も消える。動作は常に正しく、失われるのは API 課金ゼロの猶予のみ）。APP-003 SYN-04 の物理削除タイミング記述を現設計（起動時スイープ + 予約解除直前の orphan-only 回収）に整合（APP-003 側改訂）。strategy の Sweep 説明に残っていた旧方式（`DeleteSeries`）表現を修正                                                                                                                                                                                                                                                                                                                                                  |
| 2026-07-06 | 1.11       | 実装後レビュー指摘対応（P1×2 + P2×1）: (1) stale 予約の発生源遮断を §3.3 に MANDATORY として新設 — `DeleteKey` は当該 KEY の全 `pending_deletions` 行を、`DeleteSeriesAll` は当該 key+series の行を、本体削除と同一 tx で除去する（`schedule_delete_series` → `delete_index`/`delete_series` → 同名再作成 → 再起動、の順序で series-wide 予約が新データを破壊する実データ消失経路を遮断。`DeleteSeriesAll` は「無改造再利用」から予約行除去を伴う改修に変更）。(2) §3.5.2 の `delete_documents` の囲み方を「存在チェック（HasRecord）から DeleteSeries まで」に是正 — チェックをロック外に置くと sync がロック保持中に作成する path への削除要求を取りこぼす TOCTOU（SYN-08 違反）。(3) §3.4 前提条件に「空 documents は正当な desired-state として受理する」を明記（APP-003 SYN-01 の「完全な現在状態」定義に整合。`delete_series` への誘導は即時物理削除で自己修復性を失うため不採用）                           |
| 2026-07-06 | 1.12       | レビュー指摘対応（1.11 の過剰除去による orphan 永久残留）: `DeleteSeriesAll` の予約行除去を series-wide 予約（`path=''` センチネル）**のみ**に限定。1.11 の「key+series の全予約行除去」は、`DeleteSeriesAll` が `series_keys` JOIN で対象列挙するため orphan record（`series_keys` 0 件）に触れないにも関わらず、その orphan の唯一の回収手段である path 単位予約まで消してしまい、orphan が永久残留する経路（sync で切り離し → `delete_series` → 予約消失 → スイープ回収不能）を作っていた。path 単位予約は残しても `DeleteOrphanRecords` の orphan-only 性により同名 series 再作成後の live record を壊さない（無害）。§3.3 に `DeleteKey`（orphan 含む全 record 物理削除 → 全行除去可）との**非対称が意図的である**ことを明文化                                                                                                                                                                                |
