...
**stale 予約の発生源遮断 [MANDATORY]**: `DeleteKey` は当該 KEY の全 `pending_deletions` 行を、`DeleteSeriesAll` は当該 key+series の `pending_deletions` 行（path 単位・series 全体センチネルの両方）を、**それぞれ本体の削除と同一トランザクションで除去する**。これを行わないと、`schedule_delete_series(K, s)` → `delete_index(K)`（または `delete_series(K, s)`）→ 同名 KEY/series の再作成 → 再起動、の順序で stale な series-wide 予約（`path=''`）が残存し、起動時スイープの `DeleteSeriesAll` が**再作成された新データを削除する**（path 単位の stale 行は orphan-only 処理のため無害だが、series-wide 行は live record を対象にするため発生源で断つ必要がある）。なお `sync_documents` 経由の series-wide 予約解除（SYN-04）はこの遮断とは独立に機能する。
