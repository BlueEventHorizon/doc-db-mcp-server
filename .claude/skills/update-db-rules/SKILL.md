...
```bash
python3 .claude/skills/update-db-rules/scripts/run_sync.py --type rules --start-only
```
...
```bash
python3 .claude/skills/update-db-rules/scripts/docdb_client.py sync-status --job-id <job_id 上記の値>
```
...
`run_sync.py --type rules` を実行すれば、1 プロセス内で完結して最終ジョブ状態
