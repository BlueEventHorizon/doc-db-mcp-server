...
【動作】
  1. content をクライアントから受領 (または url から取得、または local_path をサーバーがディスクから読む)
  2. SHA-256 ハッシュを計算 → 既存と一致なら embedding を再利用 (series_keys に追記)
  3. 不一致なら Markdown を見出し境界でチャンク分割し、OpenAI Embedding API で
     ベクトル化して保存

【content / url / local_path の使い分け】
  - content: クライアントが本文を直接送る (旧来の使い方)
  - url: リモート HTTP(S) から取得 (サーバーが Fetcher で GET)
  - local_path: サーバーローカルファイル絶対パス。ローカル運用時に大容量ドキュメントを
                MCP payload で送らずに済む (推奨)。'..' やシンボリックリンク越しの相対
                パスは reject、10MB 上限

【注意】
  - content / url / local_path は排他 (exactly-one)。複数指定するとエラー。
...
// UpsertDocument は upsert_documents の document 1 件分入力（content / url / local_path 排他）。
type UpsertDocument struct {
	Path      string `json:"path" jsonschema:"ドキュメントの識別子。KEY+series+path の組で一意。search 結果で表示されるパスでもある。例: 'README.md', 'src/api.md'。"`
	Content   string `json:"content,omitempty" jsonschema:"ドキュメント本文の Markdown テキスト。url / local_path と排他。"`
	URL       string `json:"url,omitempty" jsonschema:"ドキュメント取得元 URL (http/https)。サーバーが取得しハッシュを算出。content / local_path と排他。"`
	LocalPath string `json:"local_path,omitempty" jsonschema:"ローカルファイルシステム上の絶対パス。doc-db サーバーがこのパスを直接読み込む。content / url と排他。ローカル運用時に大容量ドキュメントを content で送らずに済ませる用途。絶対パス必須。'..' を含むパス、シンボリックリンクを辿った先の相対パスは拒否される。"`
	Hash      string `json:"hash,omitempty" jsonschema:"コンテンツの SHA-256 ハッシュ (省略時はサーバーが算出)。content 指定時に渡すと検証に使われる。url / local_path 指定時は無視される。"`
}
...
type UpsertInput struct {
	...
	Documents []UpsertDocument `json:"documents" jsonschema:"登録するドキュメントのリスト。各要素は content / url / local_path のいずれか 1 つを指定する。ローカル運用ではサーバー側でファイルを読む local_path が payload 削減で有利。"`
}
...
func (h *Handlers) upsertOne(
	...
	// content / url / local_path は exactly-one。
	sources := 0
	if doc.Content != "" {
		sources++
	}
	...
	if sources != 1 {
		err := errors.New("content / url / local_path のいずれか 1 つを指定")
		result.Failed++
		result.Errors = append(result.Errors, UpsertError{Path: doc.Path, Error: err.Error()})
		return err
	}
	...
	case doc.LocalPath != "":
		var err error
		content, err = readLocalDocument(doc.LocalPath)
		if err != nil {
			result.Failed++
			result.Errors = append(result.Errors, UpsertError{Path: doc.Path, Error: "local_path: " + err.Error()})
			return err
		}
	}
...
// readLocalDocument は upsert_documents の local_path で指定されたローカルファイルを読み込む。
//
// セキュリティ制約:
//   - 絶対パスのみ許容（相対パスは reject）
//   - `..` 要素を含むパスは reject（path traversal 防止）
//   - シンボリックリンクは filepath.EvalSymlinks で解決した後の実パスも同条件で検証
//   - ファイルサイズ上限 10MB（暴走 payload 防止）
//
// doc-db は local-only 運用前提だが、これらの検証は「client が誤って `/etc/passwd` 等の
// システムファイルを指すのを防ぐ」ための最低限の安全策。より厳格な制限が必要なら
// 将来的に config に allowlist を追加する（YAGNI で保留）。
func readLocalDocument(path string) (string, error) {
	if path == "" {
	...
	for _, seg := range strings.Split(path, string(filepath.Separator)) {
		if seg == ".." {
			return "", fmt.Errorf("path contains '..' element: %q", path)
		}
	}
	// symlink 解決後の実パスでも `..` を再検証
	...
	for _, seg := range strings.Split(resolved, string(filepath.Separator)) {
		if seg == ".." {
			return "", fmt.Errorf("resolved path contains '..': %q", resolved)
		}
	}
	// サイズ上限チェック
	...
	if fi.Size() > maxSize {
		return "", fmt.Errorf("file too large (%d bytes, max %d)", fi.Size(), maxSize)
	}
